...
public class GemBoxCommand extends ICommand {


    public GemBoxCommand(Trove plugin) {
        super(
                plugin,
                "gembox",
                "&c/gembox give <player> <type> <amount>"
        );
    }


    private static class GemBoxGiveCommand extends ISubCommand {

        public GemBoxGiveCommand(Trove plugin) {
            super(
                    plugin,
                    "give",
                    Arrays.asList(
                            new CommandArgument(1, PlayerUtils.getPlayerNames()),
                            new CommandArgument(2, new ArrayList<>(GemRarity.getNames())),
        ...
        public void execute(CommandSender sender, String[] args) throws IOException {
            ...
            final ItemStack gemBox = ((GemBox) getPlugin().getBoxRegistry().getBox("gem_box"))
                    .makeBox(GemRarity.valueOf(args[2].toUpperCase()));
    ...
    public List<ISubCommand> getSubCommands() {
        return Arrays.asList(
                new GemBoxGiveCommand(getPlugin())
        );
    }
}
